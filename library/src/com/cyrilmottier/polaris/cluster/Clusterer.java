 ...
 * other on a {@link PolarisMapView} by projecting geolocation to a position on
 * screen and detecting pixel proximity.
 * 
 ...
 * ClusterSpot drawable. This drawable will be a certain size and colour
 * depending on how many {@link Annotation} objects it contains and the current
 * {@link ClusterConfig}
 * 
 * e.g.
 * 
 * by default a Cluster with less than 10 Annotations will be defined as Low and
 * will display as a yellow circle with a diameter of 24dips.
 * 
 ...
 * 
 * by default a Cluster with 20+ Annotations will be defined as High and will
 * display as a red circle with a diameter of 32dips.
 * 
 * These are configurable from within the Polaris colors and dimens project
 * resources.
 * 
...
public class Clusterer {
	...
	/**
	 * Cluster config used for categorising clusters into low, med & high
	 * categories. Defaults will apply if not supplied by the caller
	 **/
	 ...
	 *            A {@link PolarisMapView} object.
	 * @param map
	 *            A {@link ClusterConfig} configuration for low, med & high
	 *            cluster spots.
	 */
	 ...
	 *            A {@link PolarisMapView} object.
	 * @param items
	 *            A list of {@link Annotation} objects.
	 */
	...
	/**
	 * Add a List of {@link Annotation} to our {@link Clusterer} cache. This
	 * method will add the supplied {@link Annotation}s to our internal cache of
	 * Annotations
	 * 
	 * @param items
	 *            A list of {@link Annotation} objects.
	...
	/**
	 * Add a single {@link Annotation} to our {@link Clusterer} cache. This
	 * method will add the supplied {@link Annotation} to our internal cache of
	 * Annotations
	 * 
	 * @param item
	 *            An {@link Annotation} object.
	 */
	...
	/**
	 * Add a single {@link Annotation} to our {@link Clusterer}. This method
	 * will add the supplied {@link Annotation} to an existing cluster if it
	 * overlaps with any other {@link Cluster} that we have stored. Otherwise it
	 * will create a new {@link Cluster} object
	 * 
	 * @param item
	 *            An {@link Annotation} object.
	 */
	...
	/**
	 * Allows you to get a reduced set of {@link Annotation} objects with by
	 * supplying a new set of master Annotations
	 * 
	 * @param items
	 *            new set of Annotations
	 * @return ArrayList of clustered Annotations
	 */
	 ...
	 * through our list of {@link Cluster} objects and replacing the given
	 * marker with a Cluster spot drawable depending on how many Annotations are
	 * within the given {@link Cluster}.
	 * 
		...
		for (Cluster cluster : mClusters) {
			...
			if (total > 1) {
				...
				if (total <= mClusterConfig.getLow()) {
					marker = ClusterUtils.createClusterSpot(mContext, mClusterConfig.getLowClusterSpot(), total);
				} else if (total <= mClusterConfig.getMedium()) {
					marker = ClusterUtils.createClusterSpot(mContext, mClusterConfig.getMediumClusterSpot(), total);
				} else {
					marker = ClusterUtils.createClusterSpot(mContext, mClusterConfig.getHighClusterSpot(), total);
				}
	...
	/**
	 * Sets a custom Clusterer configuration. If null is supplied then we create
	 * one with default values
	 * 
	 * @param config
	 *            a config object to define high, med & low thresholds
	 * 
	 */
	public void setClusterConfig(ClusterConfig config) {
		mClusterConfig = config == null ? new ClusterConfig(mContext) : config;
	}
